...
// These must all be declared public for JsonConvert.SerializeObject()
	...
	public class Slim
	{
		...
		private List<string> Reconcile(List<string> vList, int car)
		{
			...
			int count = (0 == car) ? MIDIio.gCount : MIDIio.pCount;
		...
		internal bool Load(string path)
		{
			...
			if (null == data)
			{
				MIDIio.Msg = "null data";
				return true;
			}

			if (null == data.pList)
			{
				MIDIio.Msg = "null data.pList";
				return true;
			}

			if (null == data.gList)
			{
				MIDIio.Msg = "null data.gList";
				return true;
			}
			...
			//	js.OOpa($"Slim.Load({path}) data.Plugin: {data.Plugin} != MIDIio");
				data.Plugin = "JSONio";	// user has at least been warned...
			}

			int nullcarID = 0;
			int pCount = MIDIio.pCount;
			int gCount = MIDIio.gCount;
			int i, g, c;
